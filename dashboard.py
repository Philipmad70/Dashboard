...
def fetch_art(now_local):
    ...
    nyt hver dag (datoen bruges som froe)."""
    result = None
    try:
        # 1) Hent en side af public-domain vaerker med billede
        fields = "id,title,artist_display,date_display,image_id,medium_display"
        # Brug datoen som froe, saa vaerket skifter dagligt men er stabilt i dag
        seed = int(now_local.strftime("%Y%m%d"))
        page = (seed % 100) + 1   # side 1..100
        url = ("https://api.artic.edu/api/v1/artworks/search"
               "?query[term][is_public_domain]=true"
               "&fields=" + fields +
               "&limit=20&page=" + str(page))
        data = json.loads(fetch_url(url, headers={"Accept": "application/json"}))
        items = [w for w in data.get("data", []) if w.get("image_id")]
        if not items:
            print("  Kunst: ingen vaerker fundet")
            return None
        # Vaelg et vaerk ud fra froet
        art = items[seed % len(items)]
        iiif = data.get("config", {}).get("iiif_url", "https://www.artic.edu/iiif/2")
        img = f"{iiif}/{art['image_id']}/full/600,/0/default.jpg"
        result = {
            "title":  art.get("title", ""),
...
def fetch_fun_facts(now_local, onthisday=None):
    ...
    r = gemini_grounded(
        ...
        f"Lav nøgleordet eller emnet i hvert punkt til et klikbart HTML-link "
        f'til en relevant kilde, på formen <a href="URL">tekst</a>. '
        f"Brug rigtige, fungerende URL'er fra din søgning. "
...
def generate_html(day_summary, dk_prose, w_prose,
                  ...
                  wiki=None, astro=None, funfacts="", art=None, wallnot_html=""):
  ...
  <div id="top-info">
    ...
    <div id="top-news">
      ...
      <div class="news2">
        ...
        <div><div class="catlbl">&#x1F4F0; Wallnot &mdash; uden betalingsmur</div><p class="newsp" id="wallnot-list">{wallnot_html}</p></div>
      </div>
    </div>
    <div id="top-row2">
      <div id="top-wiki">
...
def main():
    ...
    html=generate_html(day_summary,dk_prose,w_prose,
        ...
        reddit_by_sub,drtv_today,drtv_yesterday,now_local,
        wiki,astro,funfacts,art,wallnot_html)
