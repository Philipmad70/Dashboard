...
def safe_prose(t):
    ...
    return re.sub(r'<(?!/?(?:a|ul|li|span)\b)[^>]+>','',s)
...
def fetch_news():
    ...
    def headline_list(items, n=12):
        # Liste med klikbare overskrifter under opsummeringen
        rows = []
        for t,l in items[:n]:
            if l:
    ...
    def summ(items, label, greet):
        if not items: return ""
        block = "\n".join(f"{i+1}. {t} | LINK: {l}" for i,(t,l) in enumerate(items[:10]))
        intro = "Begynd med 'Hej Philip, '" if greet else "Begynd direkte"
        r = gemini(f"Dansk nyhedsassistent. Overskrifter fra {label}:\n{block}\n\n"
            f"2-3 sætninger, vigtigste nyheder. Integrer links som HTML <a>. "
            f"{intro}. Neutral tone. Kun <a> tags.", 0.4)
        summary = r or " · ".join(
            f'<a href="{esc(l)}" target="_blank">{esc(t)}</a>' if l else esc(t)
            for t,l in items[:5])
        # Opsummering først, derefter en liste med flere klikbare overskrifter
        return f'<span class="news-summary">{summary}</span>{headline_list(items)}'
    print("  Nyheder OK")
