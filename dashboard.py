...
def fetch_wikipedia_featured(now_local):
    ...
    for ev in (data.get("onthisday") or [])[:30]:
        year = ev.get("year")
        text = ev.get("text", "")
        if year and text:
            result["onthisday"].append({"year": year, "text": text})
...
def fetch_fun_facts(now_local, onthisday):
    ...
    r = gemini(
        ...
        f"Skriv 2-3 korte, sjove eller overraskende fakta om denne dato "
        f"(mærkedage, traditioner, kuriositeter, eller noget der skete historisk). "
        f"Skriv på letforståeligt dansk. Brug punktopstilling med <li>-tags inde i "
        f"en <ul>. Ingen overskrift, kun listen. Ingen markdown.", 0.7)
    return r or ""


def summarize_day(cal_today, now_local):
    ...
    r = gemini(f"Du er Philips assistent. I dag: {dk_date(now_local.date())} {now_local.year}.\n"
               f"Kalender:\n{block}\n\n2-3 sætninger. Nævn HVERT møde med præcist navn og tid. "
...
def generate_html(day_summary, dk_prose, w_prose,
    ...
    # ── Astronomi-linje til "Din dag" ──
    astro = astro or {}
    astro_html = ""
    bits = []
    if astro.get("sunrise") and astro.get("sunset"):
        bits.append(f'<span class="astro-item">&#x1F305; Sol {esc(astro["sunrise"])}&ndash;{esc(astro["sunset"])}</span>')
    if astro.get("moon_phase"):
        bits.append(f'<span class="astro-item">{astro.get("moon_icon","")} {esc(astro["moon_phase"])}</span>')
    if bits:
        astro_html = '<div class="astro-row">' + "".join(bits) + '</div>'

    # ── Sjove fakta (AI) til "Din dag" ──
    funfacts_html = ""
    if funfacts:
        ff = safe_prose(funfacts)
        funfacts_html = f'<div class="funfacts"><div class="mini-label">&#x2728; Sjove fakta om i dag</div>{ff}</div>'

    # ── "On this day" historiske begivenheder til "Din dag" ──
    wiki = wiki or {"tfa": None, "onthisday": []}
    otd_html = ""
    otd = wiki.get("onthisday", [])[:6]
    if otd:
        rows = "".join(f'<li><span class="otd-year">{e["year"]}</span> {esc(e["text"])}</li>' for e in otd)
...
def main():
    ...
    day_summary=summarize_day(events_for_day(cal_events,today),now_local)
