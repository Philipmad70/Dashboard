...
def gemini(prompt, temp=0.5):
    try:
        ...
        try: msg = e.read().decode()[:80]
        except Exception: msg = ""
        print(f"  Gemini {e.code}: {msg}"); return ""
    except Exception as e:
        # Timeout eller andre netværksfejl må ALDRIG vælte hele scriptet
        print(f"  Gemini fejl (springes over): {e}"); return ""
...
def gemini_grounded(prompt, temp=0.6):
    ...
    try:
        ...
        print(f"  Gemini (søg) {e.code}: {msg}"); return ""
    except Exception as e:
        print(f"  Gemini (søg) fejl (springes over): {e}"); return ""
...
def fetch_news():
    ...
    # Wallnot — danske nyheder uden paywall (ren liste, ikke opsummering)
    wallnot_items = fetch_rss("https://wallnot.dk/rss", n=12)
    if wallnot_items:
        rows = "".join(
            f'<li><a href="{esc(l)}" target="_blank" rel="noopener">{esc(clean_title(t))}</a></li>'
            for t, l in wallnot_items[:10] if t)
        wallnot_html = f'<ul class="news-headlines">{rows}</ul>'
    else:
        wallnot_html = '<span class="dimtxt">Ingen nyheder</span>'

    print("  Nyheder OK")
    return summ(dk,"Danmark",True), summ(wo,"Verden",False), wallnot_html

# ═══════════════════════════════════════════════════════
#  📚 WIKIPEDIA — dagens artikel + "on this day"
# ═══════════════════════════════════════════════════════
def fetch_art(now_local):
    """Dagens kunstvaerk fra Art Institute of Chicago. Samme vaerk hele dagen,
    nyt hver dag (datoen bruges som froe). Stoerre billede + robust fallback."""
    result = None
    seed = int(now_local.strftime("%Y%m%d"))
    fields = "id,title,artist_display,date_display,image_id,medium_display"
    iiif_default = "https://www.artic.edu/iiif/2"
    try:
        # Lavt sidetal (1-25) for at undgaa tomme sider; froe styrer hvilken side
        page = (seed % 25) + 1
        url = ("https://api.artic.edu/api/v1/artworks/search"
               "?query[term][is_public_domain]=true"
               "&fields=" + fields +
               "&limit=30&page=" + str(page))
        data = json.loads(fetch_url(url, headers={"Accept": "application/json"}, timeout=25))
        items = [w for w in data.get("data", []) if w.get("image_id")]
        iiif = data.get("config", {}).get("iiif_url", iiif_default)
        if items:
            art = items[seed % len(items)]
            # Stoerre billede: 1200 px bredt (var 600)
            img = f"{iiif}/{art['image_id']}/full/1200,/0/default.jpg"
            result = {
                "title":  art.get("title", ""),
            ...
            print(f"  Kunst: {result['title']}")
            return result
        print("  Kunst: ingen vaerker paa side, proever kendt vaerk")
    except Exception as e:
        print(f"  Kunst (soegning fejlede): {e}")

    # Fallback: en lille liste kendte mestervaerker (id'er fra AIC)
    try:
        masterpieces = [27992, 28560, 20684, 111628, 64818, 16568, 109275,
                        14598, 80607, 4884, 117266, 100829]
        aid = masterpieces[seed % len(masterpieces)]
        url = f"https://api.artic.edu/api/v1/artworks/{aid}?fields={fields}"
        data = json.loads(fetch_url(url, headers={"Accept": "application/json"}, timeout=25))
        art = data.get("data", {})
        iiif = data.get("config", {}).get("iiif_url", iiif_default)
        if art.get("image_id"):
            result = {
                ...
                "date":   art.get("date_display", ""),
                "medium": art.get("medium_display", ""),
                "image":  f"{iiif}/{art['image_id']}/full/1200,/0/default.jpg",
                "url":    f"https://www.artic.edu/artworks/{aid}",
            }
            print(f"  Kunst (fallback): {result['title']}")
    except Exception as e:
...
def fetch_wikipedia_featured(now_local):
    ...
    tfa = data.get("tfa") or {}
    if tfa:
        # Foretræk originalbilledet (stort) frem for thumbnail (lille)
        thumb = ((tfa.get("originalimage") or {}).get("source", "")
                 or (tfa.get("thumbnail") or {}).get("source", ""))
        result["tfa"] = {
...
# ═══════════════════════════════════════════════════════
def fetch_local_events(now_local):
    """Finder lokale begivenheder nær brugeren (København-området) via søgning.
    Returnerer kort rå tekst, der væves ind i dagsopsummeringen."""
    dato = f"{now_local.day}. {DK_MONTHS[now_local.month]} {now_local.year}"
    r = gemini_grounded(
        f"I dag er det {dato}. Brug søgning til at finde 3-4 konkrete "
        f"begivenheder og arrangementer i København og omegn (inkl. Hvidovre, "
        f"Frederiksberg, Amager) i dag eller de kommende dage. "
        f"Prioritér loppemarkeder og kræmmermarkeder højt, men tag også gerne "
        f"koncerter, udstillinger, markeder, foredrag eller festivaler med. "
        f"For hver: skriv kun navn, sted og hvornår — meget kort. "
        f"Svar som en simpel liste, ét arrangement per linje, uden tal, "
        f"uden bullets, uden links, uden HTML. Kun ren tekst. "
        f"Hvis du intet finder, svar med et tomt svar.", 0.5)
    return r or ""
...
def summarize_day(cal_today, now_local, astro=None, local_events=""):
    ...
    events_txt = ""
    if local_events and local_events.strip():
        events_txt = "Lokale arrangementer i nærheden:\n" + local_events.strip()
    r = gemini(f"Du er Philips assistent. I dag: {dk_date(now_local.date())} {now_local.year}.\n"
               f"Kalender:\n{block}\n{sky}\n{events_txt}\n\n"
               "Skriv i 3-4 korte afsnit adskilt af en tom linje. "
               "Første afsnit: en hilsen og dagens møder (nævn HVERT møde med præcist navn og tid). "
               "Andet afsnit: solopgang/solnedgang og månefasen vævet ind som prosa. "
               "Tredje afsnit: begynd med 'I dag i nærheden af dig sker der...' og nævn de "
               "lokale arrangementer naturligt i prosa (kun hvis der er nogen ovenfor). "
               "Et kort sidste afsnit med en opmuntrende afslutning. "
               "Begynd 'Hej Philip, '. Adskil afsnit med en tom linje. Ingen markdown.", 0.6)
    if r: return r
    if not cal_today:
...
def render_reddit_post(p, now_local):
  ...
  <div class="rpost-text">
    ...
    {ext_html}
    <span class="rmeta">{esc(ages)}</span>
  </div>
  {img_html}
</a>"""
    ext = p.get("ext_link","")
...
def generate_html(day_summary, dk_prose, w_prose,
                  ...
                  wiki=None, astro=None, art=None, wallnot_html=""):
    ...
    # Del dagsopsummeringen i afsnit (adskilt af tomme linjer)
    _paras = [p.strip() for p in re.split(r"\n\s*\n", day_summary or "") if p.strip()]
    if _paras:
        day_summary_html = "".join(f"<p>{esc(p)}</p>" for p in _paras)
    else:
        day_summary_html = f"<p>{esc(day_summary)}</p>"
  ...
  <div id="top-info">
    <div id="top-row1">
      <div id="top-sum">
        ...
        <div class="sumtxt">{day_summary_html}</div>
      </div>
      <div></div>
      <div id="top-wx">
    ...
    <div id="top-news">
      ...
      <div class="news2">
        ...
        <div><div class="catlbl">&#x1F4F0; Andre nyheder &mdash; fra Wallnot</div><p class="newsp" id="wallnot-list">{wallnot_html}</p></div>
      </div>
    </div>
    <div id="top-row2">
      <div id="top-wiki">
      ...
      <div id="top-art">
        <div class="top-section-label">&#x1F3A8; Dagens kunstv&aelig;rk</div>
        {art_html}
      </div>
    </div>
  </div>
...
def main():
    ...
    print("📍 Henter lokale begivenheder …")
    local_events=fetch_local_events(now_local)
    print("🌅 Dagsopsummering …")
    day_summary=summarize_day(events_for_day(cal_events,today),now_local,astro,local_events)
