...
def fetch_local_events(now_local):
    ...
    loppemarkeder, markeder, koncerter, udstillinger, arrangementer mm."""
    dato = f"{now_local.day}. {DK_MONTHS[now_local.month]} {now_local.year}"
    r = gemini_grounded(
        f"I dag er det {dato}. Brug søgning til at finde 4-6 konkrete "
        ...
        f"andre ting med: koncerter, udstillinger, markeder, byvandringer, "
        f"foredrag, festivaler, sportsbegivenheder eller familieaktiviteter. "
        f"For hver begivenhed: skriv kort hvad det er, hvor og hvornår. "
        f"Skriv på letforståeligt dansk. Hver begivenhed som ét punkt. "
        f"Lav navnet på begivenheden til et klikbart HTML-link til en relevant "
        f'kilde, på formen <a href="URL">navn</a>. Brug rigtige URL\'er fra søgningen. '
        f"Brug punktopstilling med <li>-tags inde i en <ul>. "
        f"Ingen overskrift, kun listen. Ingen markdown (brug HTML-tags).", 0.5)
    return r or ""
...
def summarize_day(cal_today, now_local, astro=None):
    ...
    r = gemini(f"Du er Philips assistent. I dag: {dk_date(now_local.date())} {now_local.year}.\n"
               f"Kalender:\n{block}\n{sky}\n\nSkriv i 2-3 korte afsnit adskilt af en tom linje. "
               ...
               "Eventuelt et kort tredje afsnit med en opmuntrende afslutning. "
...
def generate_html(day_summary, dk_prose, w_prose,
                  ...
                  wiki=None, astro=None, local_events="", art=None, wallnot_html=""):
    ...
    events_html = ""
    if local_events:
        ev = safe_prose(local_events)
        events_html = f'<div class="funfacts"><div class="mini-label">&#x1F4CD; I n&aelig;rheden &mdash; loppemarkeder &amp; arrangementer</div>{ev}</div>'
...
def main():
    ...
    html=generate_html(day_summary,dk_prose,w_prose,
        ...
        reddit_by_sub,drtv_today,drtv_yesterday,now_local,
        wiki,astro,local_events,art,wallnot_html)
